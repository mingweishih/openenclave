    ...
    uint64_t flags)
{
    ...
    if (!_layout_entries_maxium)
        _layout_entries_maxium = enclave->layout_entries_size / sizeof(oe_sgx_enclave_layout_t);
    ...
    if (_layout_entries_index > 0)
    {
        ...
        {
            printf("Found gap between 0x%lx and 0x%lx (%lu pages)\n",
                   address, previous_address, (address - previous_address) / OE_PAGE_SIZE);
            //OE_RAISE(OE_UNEXPECTED);
        }
    }

#ifdef OE_LAYOUT_DEBUG
    printf("[add layout entry] #%zu addr=0x%lx, size=%zu, type: %u, permission: %u\n",
           _layout_entries_index, address, size, type, permission);
#endif
