    ...
    bool extend)
{
    ...
    for (i = 0; i < npages; i++)
    {
        ...
        OE_CHECK(oe_sgx_add_enclave_layout_entry(
            enclave, addr, OE_PAGE_SIZE, flags));
    ...
    uint64_t* vaddr)
{
    ...
    printf(
        "[_add_layout_entries_pages] oe_sgx_layout_t size: %zu, layout size: "
        "%zu, page number: %zu\n",
    ...
    size_t* enclave_layout_entries_size)
{
    ...
    *loaded_enclave_pages_size = 0;
    ...
    layout_entreis_size = oe_round_up_to_page_size(
        total_size / OE_PAGE_SIZE * sizeof(oe_sgx_enclave_layout_t));
    ...
    oe_enclave_t* enclave)
{
    ...
    {
        enclave->layout_entries = (oe_sgx_enclave_layout_t*)oe_memalign(
            OE_PAGE_SIZE, layout_entreis_size);
