...
int (*_enclave_modify)(uint64_t addr, size_t length, int flags_from, int flags_to);
...
static void _load_sgx_enclave_common_impl(void)
{
    ...
    if (_module)
    {
        ...
        OE_CHECK(_lookup_function(
            "enclave_alloc", (void**)&_enclave_alloc));
        OE_CHECK(_lookup_function(
            "enclave_modify", (void**)&_enclave_modify));
...
int oe_sgx_enclave_modify(uint64_t addr, size_t length, int flags_from, int flags_to)
{
