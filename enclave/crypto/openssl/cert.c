 ...
 * takes strings. The function decodes the oid in hex format into its string
 * format.*/
static char* _decode_oid_to_str(char* oid, int oid_size)
{
    char byte;
    char* oid_str;
    ...
    {
        num = 0;
        byte = oid[hi++] & (char)0xff;
        while ((byte & (1 << 8)) >> 8)
        { // if highest bit is one, there are following bytes
            byte = byte & 0b1111111; // remove highest bit
            num = (num + byte) << 7; // add up
            byte = oid[hi++];
        ...
        if ((unsigned long)i + strlen(num_str_buf) > 100) // string too long
            return NULL;
    ...
    // Copy from buffer
    oid_str = (char*)malloc(((unsigned long)i + 1) * sizeof(char));
    ...
    size_t* bytes_written)
{
    ...
    strcpy(date_str, (const char*)config->date_not_valid_after);
    strcat(date_str, "Z");
    ...
    txt = _decode_oid_to_str((char*)config->ext_oid, (int)config->ext_oid_size);
    if (!txt)
    {
