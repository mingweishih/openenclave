...
// but which still needs to be known to the compiler to be able to use the
// library. This includes structure declarations and all support for inline
// implementations of some of the library functions. Information in this file is
...
//
// SYMCRYPT_CALL is a macro that selects the calling convention used by the
// library. Crypto functions often have to perform very many small operations,
// and a fast calling convention is preferable. We use __fastcall on platforms
// that support it.
//
// SYMCRYPT_ALIGN is the default alignment for the platform.
// On platforms that have alignment restrictions the default alignment should be
// large enough that an aligned BYTE * can be cast to a pointer to a UINT32 and
// be used.
//
//
// The SYMCRYPT_IGNORE_PLATFORM macro can be defined to switch off any
// platform-specific optimizations and run just the C implementations. The rest
// of the library uses SYMCRYPT_CPU_* macros to make platform decisions.
//
//
// WARNING: both the library and the calling application must be compiled with
// the same set of flags, as the flags affect things like the structure layout
// and size and the calling convention, both of which need to be in sync between
// the lib and the caller.
//

//#define SYMCRYPT_IGNORE_PLATFORM        // #defining this flag disables all
...
#pragma warning(push)
#pragma warning( \
    disable : 4359) // *** Alignment specifier is less than actual alignment
#endif

#elif (defined(_ARM64_) || defined(_ARM64EC_)) && \
    !defined(SYMCRYPT_IGNORE_PLATFORM)
...
#define SYMCRYPT_ALIGN_UP(_p)                         \
    ((PBYTE)(                                         \
        ((UINT_PTR)(_p) + SYMCRYPT_ALIGN_VALUE - 1) & \
        ~(SYMCRYPT_ALIGN_VALUE - 1)))
...
#pragma prefast(     \
    disable : 28251, \
    "Windows headers define _mm_clflush with SAL annotation, Intel header doesn't have SAL annotation leading to inconsistent annotation errors")
#include <emmintrin.h>
#pragma prefast(pop)
#endif
...
// Where needed the library provides for copy functions of its internal data
// structures.
//
#if SYMCRYPT_DEBUG
...
#define SYMCRYPT_SET_MAGIC(p)                 \
    {                                         \
        (p)->magic = SYMCRYPT_MAGIC_VALUE(p); \
    }
#define SYMCRYPT_CHECK_MAGIC(p)                    \
    {                                              \
        if ((p)->magic != SYMCRYPT_MAGIC_VALUE(p)) \
            SymCryptFatal('magc');                 \
    }
...
// hard-to-debug problems with people who accidentally mix FRE headers with
// CHKed libraries, or the other way around. E.g. BitLocker only publishes the
// FRE version of their library, and building a CHKed binary with that FRE lib
// crashes
//
...
//
// We keep the most commonly tested bits in the least significant byte, to make
// it easier for the compiler to optimize There is a many to one relationship
// between CPUID feature flags and SYMCRYPT_CPU_FEATURE_XXX bits since a
// SYMCRYPT_CPU_FEATURE_XXX could require multiple CPUID features.
...
#define SYMCRYPT_CPU_FEATURE_SAVEXMM_NOFAIL \
    0x0020 // if SymCryptSaveXmm() will never fail
...
// g_SymCryptCpuFeaturesNotPresent reports with features are not present on the
// current CPU SymCryptCpuFeaturesNeverPresent() is a function that returns a
// static (compiler-predictable) value,
//  and allows the environment to lock out features in a way that the compiler
//  can optimize away all the code that uses these features.
// Using a function allows the environment macro to forward it to an
...
#define SYMCRYPT_CPU_FEATURES_PRESENT(x)             \
    (((x)&SymCryptCpuFeaturesNeverPresent()) == 0 && \
     ((x)&g_SymCryptCpuFeaturesNotPresent) == 0)
...
// access away. They provide no other memory ordering requirements, so there are
// no acquire/release semantics, memory barriers, etc.
//
// For Windows (MS_VC compiler) we use the provided by the OS functions.
//
// The generic versions are implemented with a volatile access, but that is
// inefficient on some platforms because it might introduce memory ordering
// requirements.
//
...
#if SYMCRYPT_CPU_ARM || SYMCRYPT_CPU_ARM64
#define SYMCRYPT_INTERNAL_FORCE_READ8(_p) \
    (__iso_volatile_load8((const volatile char*)(_p)))
#define SYMCRYPT_INTERNAL_FORCE_READ16(_p) \
    (__iso_volatile_load16((const volatile short*)(_p)))
#define SYMCRYPT_INTERNAL_FORCE_READ32(_p) \
    (__iso_volatile_load32((const volatile int*)(_p)))
#define SYMCRYPT_INTERNAL_FORCE_READ64(_p) \
    (__iso_volatile_load64((const volatile __int64*)(_p)))

#define SYMCRYPT_INTERNAL_FORCE_WRITE8(_p, _v) \
    (__iso_volatile_store8((volatile char*)(_p), (_v)))
#define SYMCRYPT_INTERNAL_FORCE_WRITE16(_p, _v) \
    (__iso_volatile_store16((volatile short*)(_p), (_v)))
#define SYMCRYPT_INTERNAL_FORCE_WRITE32(_p, _v) \
    (__iso_volatile_store32((volatile int*)(_p), (_v)))
#define SYMCRYPT_INTERNAL_FORCE_WRITE64(_p, _v) \
    (__iso_volatile_store64((volatile __int64*)(_p), (_v)))
#elif SYMCRYPT_CPU_X86 || SYMCRYPT_CPU_AMD64
...
#define SYMCRYPT_INTERNAL_FORCE_WRITE16(_p, _v) \
    (*((volatile UINT16*)(_p)) = (_v))
#define SYMCRYPT_INTERNAL_FORCE_WRITE32(_p, _v) \
    (*((volatile UINT32*)(_p)) = (_v))
#define SYMCRYPT_INTERNAL_FORCE_WRITE64(_p, _v) \
    (*((volatile UINT64*)(_p)) = (_v))
#else // Temporary workaround for CMake compilation issues on Windows. Assume
      // X86/ADM64.
#define SYMCRYPT_INTERNAL_FORCE_READ8(_p) (*((const volatile BYTE*)(_p)))
#define SYMCRYPT_INTERNAL_FORCE_READ16(_p) (*((const volatile UINT16*)(_p)))
#define SYMCRYPT_INTERNAL_FORCE_READ32(_p) (*((const volatile UINT32*)(_p)))
...
// We do this by platform because it affected by both endianness and alignment
// requirements The p pointer is always a pointer to BYTE
//
...
//
// X86, AMD64, ARM, and ARM64 have no alignment restrictions, and are
// little-endian. We do straight store/loads with BSWAPs where required. This
// technically relies upon on undefined behavior, as we assume the compiler will
// translate operations on unaligned pointers to 2, 4, and 8 bytes types to
// appropriately unaligned store/load instructions on these platforms (not just
// in these macros). This works for all compilers we currently use.
//
...
#define SYMCRYPT_INTERNAL_STORE_MSBFIRST16(p, x) \
    (*(UINT16*)(p) = SYMCRYPT_BSWAP16(x))
#define SYMCRYPT_INTERNAL_STORE_LSBFIRST16(p, x) (*(UINT16*)(p) = (x))
#define SYMCRYPT_INTERNAL_STORE_MSBFIRST32(p, x) \
    (*(UINT32*)(p) = SYMCRYPT_BSWAP32(x))
#define SYMCRYPT_INTERNAL_STORE_LSBFIRST32(p, x) (*(UINT32*)(p) = (x))
#define SYMCRYPT_INTERNAL_STORE_MSBFIRST64(p, x) \
    (*(UINT64*)(p) = SYMCRYPT_BSWAP64(x))
#define SYMCRYPT_INTERNAL_STORE_LSBFIRST64(p, x) (*(UINT64*)(p) = (x))

#else // unknown platform

//
// These functions have to handle arbitrary alignments too, so we do them
// byte-by-byte in the generic case. So far these macros have not been fully
// tested
//
#define SYMCRYPT_INTERNAL_LOAD_MSBFIRST16(p) \
    (((UINT16)((PBYTE)p)[0]) << 8 | ((PBYTE)p)[1])
#define SYMCRYPT_INTERNAL_LOAD_LSBFIRST16(p) \
    (((UINT16)((PBYTE)p)[1]) << 8 | ((PBYTE)p)[0])
#define SYMCRYPT_INTERNAL_LOAD_MSBFIRST32(p)                           \
    ((UINT32)SYMCRYPT_INTERNAL_LOAD_MSBFIRST16(&((PBYTE)p)[0]) << 16 | \
     SYMCRYPT_INTERNAL_LOAD_MSBFIRST16(&((PBYTE)p)[2]))
#define SYMCRYPT_INTERNAL_LOAD_LSBFIRST32(p)                           \
    ((UINT32)SYMCRYPT_INTERNAL_LOAD_MSBFIRST16(&((PBYTE)p)[2]) << 16 | \
     SYMCRYPT_INTERNAL_LOAD_MSBFIRST16(&((PBYTE)p)[0]))
#define SYMCRYPT_INTERNAL_LOAD_MSBFIRST64(p)                           \
    ((UINT64)SYMCRYPT_INTERNAL_LOAD_MSBFIRST32(&((PBYTE)p)[0]) << 32 | \
     SYMCRYPT_INTERNAL_LOAD_MSBFIRST32(&((PBYTE)p)[4]))
#define SYMCRYPT_INTERNAL_LOAD_LSBFIRST64(p)                           \
    ((UINT64)SYMCRYPT_INTERNAL_LOAD_LSBFIRST32(&((PBYTE)p)[4]) << 32 | \
     SYMCRYPT_INTERNAL_LOAD_LSBFIRST32(&((PBYTE)p)[0]))
...
#define SYMCRYPT_INTERNAL_STORE_MSBFIRST64(p, x)                         \
    {                                                                    \
        SYMCRYPT_INTERNAL_STORE_MSBFIRST32(                              \
            &((PBYTE)p)[0], (UINT32)(((UINT64)(x)) >> 32));              \
...
#define SYMCRYPT_INTERNAL_STORE_LSBFIRST64(p, x)                           \
    {                                                                      \
        ...
        SYMCRYPT_INTERNAL_STORE_LSBFIRST32(                                \
            &((PBYTE)p)[4], (UINT32)(((UINT64)(x)) >> 32));                \
    }
...
//==============================================================================================
//
// Note: we do not use the symbolic names like SYMCRYPT_SHA1_INPUT_BLOCK_SIZE as
// this file is included before that name is defined. Fixing that would make the
// public API header file harder to read by moving the constant away from the
// associated functions, or forcing the header file to use the struct name
// rather than the typedef. The current solution works quite well.
//

//-----------------------------------------------------------------
//     Block cipher description table
// Below are the typedefs for the block cipher description table type
// Callers can use this to define their own block cipher and use the block
// cipher modes.
//

typedef struct _SYMCRYPT_BLOCKCIPHER SYMCRYPT_BLOCKCIPHER,
    *PSYMCRYPT_BLOCKCIPHER;
typedef const SYMCRYPT_BLOCKCIPHER* PCSYMCRYPT_BLOCKCIPHER;

//
// Note that blockSize must be <= 32 and must be a power of two. This is true
// for all the block ciphers implemented in SymCrypt.
//

//
// HASH STATES
//
// All hash states have the same basic structure. This allows all hash
// implementations to share the same buffer management code. Some algorithms
// might still have optimized buffer management code specific for their
// algorithm, but most algs use the generic code. This is especially important
...
typedef SYMCRYPT_ALIGN_STRUCT _SYMCRYPT_COMMON_HASH_STATE
{
    ...
    SYMCRYPT_ALIGN BYTE
        buffer[SYMCRYPT_ANYSIZE_ARRAY]; // Size depends on algorithm
    // ...
    // Chaining state                                       // type/location
...
typedef struct _SYMCRYPT_PARALLEL_HASH SYMCRYPT_PARALLEL_HASH,
    *PSYMCRYPT_PARALLEL_HASH;
...
typedef VOID(SYMCRYPT_CALL* PSYMCRYPT_HASH_APPEND_FUNC)(
    PVOID pState,
    PCBYTE pbData,
    SIZE_T cbData);
typedef VOID(
    SYMCRYPT_CALL* PSYMCRYPT_HASH_RESULT_FUNC)(PVOID pState, PVOID pbResult);
typedef VOID(SYMCRYPT_CALL* PSYMCRYPT_HASH_APPEND_BLOCKS_FUNC)(
    PVOID pChain,
    PCBYTE pbData,
    SIZE_T cbData,
    SIZE_T* pcbRemaining);
...
typedef struct _SYMCRYPT_PARALLEL_HASH_OPERATION
    SYMCRYPT_PARALLEL_HASH_OPERATION,
    *PSYMCRYPT_PARALLEL_HASH_OPERATION;
typedef const SYMCRYPT_PARALLEL_HASH_OPERATION*
    PCSYMRYPT_PARALLEL_HASH_OPERATION;
...
struct _SYMCRYPT_PARALLEL_HASH_OPERATION
{
    ...
    _Field_size_(
        cbBuffer) PBYTE pbBuffer; // data to be hashed, or result buffer
    SIZE_T cbBuffer;              // size of pbData buffer.
    PSYMCRYPT_PARALLEL_HASH_OPERATION
        next; // internal scratch space; do not use.
};

SYMCRYPT_ALIGN_STRUCT
    _SYMCRYPT_PARALLEL_HASH_SCRATCH_OPERATION; // as yet unspecified struct
typedef struct _SYMCRYPT_PARALLEL_HASH_SCRATCH_OPERATION
    SYMCRYPT_PARALLEL_HASH_SCRATCH_OPERATION,
    *PSYMCRYPT_PARALLEL_HASH_SCRATCH_OPERATION;
...
typedef SYMCRYPT_ALIGN_STRUCT _SYMCRYPT_PARALLEL_HASH_SCRATCH_STATE
{
    ...
    PCBYTE pbData; // data/size of ongoing append operation; this op has already
                   // been removed from the next linked list
    SIZE_T cbData;
}
...
// - an array of SYMCRYPT_PARALLEL_HASH_SCRATCH_STATE structures, aligned to
// SYMCRYPT_ALIGN_VALUE.
// - the work array, an array of pointers to
// SYMCRYPT_PARALLEL_HASH_SCRATCH_STATEs.
// - an array of 4 + 8 + 64 SIMD vector elements, aligned to the size of those
// elements.
//
//
#if SYMCRYPT_CPU_X86 | SYMCRYPT_CPU_AMD64
...
#define SYMCRYPT_PARALLEL_SHA256_FIXED_SCRATCH                                \
    ((4 + 8 + 64) * SYMCRYPT_SIMD_ELEMENT_SIZE + SYMCRYPT_SIMD_ELEMENT_SIZE - \
     1 + SYMCRYPT_ALIGN_VALUE - 1)
#define SYMCRYPT_PARALLEL_SHA384_FIXED_SCRATCH                                \
    ((4 + 8 + 80) * SYMCRYPT_SIMD_ELEMENT_SIZE + SYMCRYPT_SIMD_ELEMENT_SIZE - \
     1 + SYMCRYPT_ALIGN_VALUE - 1)
#define SYMCRYPT_PARALLEL_SHA512_FIXED_SCRATCH                                \
    ((4 + 8 + 80) * SYMCRYPT_SIMD_ELEMENT_SIZE + SYMCRYPT_SIMD_ELEMENT_SIZE - \
     1 + SYMCRYPT_ALIGN_VALUE - 1)
#define SYMCRYPT_PARALLEL_HASH_PER_STATE_SCRATCH    \
    (sizeof(SYMCRYPT_PARALLEL_HASH_SCRATCH_STATE) + \
     sizeof(PSYMCRYPT_PARALLEL_HASH_SCRATCH_STATE))
...
typedef BOOLEAN(SYMCRYPT_CALL* PSYMCRYPT_PARALLEL_HASH_RESULT_FUNC)(
    PCSYMCRYPT_PARALLEL_HASH pParHash,
    PSYMCRYPT_COMMON_HASH_STATE pState,
    PSYMCRYPT_PARALLEL_HASH_SCRATCH_STATE pScratch,
    BOOLEAN* pRes);
typedef VOID(SYMCRYPT_CALL* PSYMCRYPT_PARALLEL_HASH_RESULT_DONE_FUNC)(
    PCSYMCRYPT_PARALLEL_HASH pParHash,
    PSYMCRYPT_COMMON_HASH_STATE pState,
...
typedef const SYMCRYPT_HMAC_SHA1_EXPANDED_KEY*
...
typedef const SYMCRYPT_HMAC_SHA256_EXPANDED_KEY*
...
typedef const SYMCRYPT_HMAC_SHA384_EXPANDED_KEY*
...
typedef const SYMCRYPT_HMAC_SHA512_EXPANDED_KEY*
...
typedef SYMCRYPT_ALIGN_STRUCT _SYMCRYPT_AES_EXPANDED_KEY
{
    ...
    BYTE(*lastEncRoundKey)
    [4][4];                       // Pointer to last encryption round key
...
typedef SYMCRYPT_ERROR(SYMCRYPT_CALL* PSYMCRYPT_MAC_EXPAND_KEY)(
    ...
    SIZE_T cbKey);
typedef VOID(
    SYMCRYPT_CALL* PSYMCRYPT_MAC_INIT)(PVOID pState, PCVOID pExpandedKey);
typedef VOID(SYMCRYPT_CALL* PSYMCRYPT_MAC_APPEND)(
    PVOID pState,
    PCBYTE pbData,
    SIZE_T cbData);
...
typedef struct _SYMCRYPT_MAC
{
    ...
    UINT32
        outerChainingStateOffset; // Offset into expanded key of outer chaining
                                  // state; 0 for non-HMAC algorithms
...
typedef SYMCRYPT_ALIGN_STRUCT _SYMCRYPT_CCM_STATE
{
    ...
    SYMCRYPT_ALIGN BYTE
        counterBlock[SYMCRYPT_CCM_BLOCK_SIZE]; // Current counter block value
    SYMCRYPT_ALIGN BYTE
        macBlock[SYMCRYPT_CCM_BLOCK_SIZE]; // Current state of the CBC-MAC part
                                           // of CCM
    SYMCRYPT_ALIGN BYTE
        keystreamBlock[SYMCRYPT_CCM_BLOCK_SIZE]; // Remaining key stream if
                                                 // partial block has been
                                                 // processed
    SYMCRYPT_MAGIC_FIELD
}
...
#define SYMCRYPT_GCM_BLOCKCIPHER_KEY_SIZE \
    sizeof(union _SYMCRYPT_GCM_SUPPORTED_BLOCKCIPHER_KEYS)
...
typedef SYMCRYPT_ALIGN_STRUCT _SYMCRYPT_GHASH_EXPANDED_KEY
{
    ...
    UINT32 tableOffset;
    BYTE tableSpace
        [(SYMCRYPT_GF128_FIELD_SIZE + 1) * sizeof(SYMCRYPT_GF128_ELEMENT)];
#else
...
typedef SYMCRYPT_ERROR(SYMCRYPT_CALL* PSYMCRYPT_BLOCKCIPHER_EXPAND_KEY)(
    ...
    PCBYTE pbKey,
    SIZE_T cbKey);
typedef VOID(SYMCRYPT_CALL* PSYMCRYPT_BLOCKCIPHER_CRYPT)(
    PCVOID pExpandedKey,
    PCBYTE pbSrc,
    PBYTE pbDst);
typedef VOID(SYMCRYPT_CALL* PSYMCRYPT_BLOCKCIPHER_CRYPT_ECB)(
    PCVOID pExpandedKey,
    PCBYTE pbSrc,
    PBYTE pbDst,
    SIZE_T cbData);
typedef VOID(SYMCRYPT_CALL* PSYMCRYPT_BLOCKCIPHER_CRYPT_MODE)(
    PCVOID pExpandedKey,
    PBYTE pbChainingValue,
    PCBYTE pbSrc,
    PBYTE pbDst,
    SIZE_T cbData);
typedef VOID(SYMCRYPT_CALL* PSYMCRYPT_BLOCKCIPHER_MAC_MODE)(
    PCVOID pExpandedKey,
    PBYTE pbChainingValue,
    PCBYTE pbSrc,
    SIZE_T cbData);
typedef VOID(SYMCRYPT_CALL* PSYMCRYPT_BLOCKCIPHER_AEADPART_MODE)(
    PVOID pState,
    PCBYTE pbSrc,
    PBYTE pbDst,
    SIZE_T cbData);
...
struct _SYMCRYPT_BLOCKCIPHER
{
    ...
    PSYMCRYPT_BLOCKCIPHER_CRYPT_ECB
        ecbEncryptFunc; // NULL if no optimized version available
    PSYMCRYPT_BLOCKCIPHER_CRYPT_ECB
        ecbDecryptFunc; // NULL if no optimized version available
    ...
    PSYMCRYPT_BLOCKCIPHER_MAC_MODE
        cbcMacFunc; // NULL if no optimized version available
    PSYMCRYPT_BLOCKCIPHER_CRYPT_MODE
        ctrMsb64Func; // NULL if no optimized version available
    PSYMCRYPT_BLOCKCIPHER_AEADPART_MODE
        gcmEncryptPartFunc; // NULL if no optimized version available
    PSYMCRYPT_BLOCKCIPHER_AEADPART_MODE
        gcmDecryptPartFunc; // NULL if no optimized version available
    _Field_range_(1, SYMCRYPT_MAX_BLOCK_SIZE) SIZE_T
        blockSize; // = SYMCRYPT_XXX_BLOCK_SIZE, power of 2, 1 <= value <= 32.
...
//
// Some CPUs like the S array type to be larger than BYTE. We abstract the data
// type of the S array to accommodate such CPUs in future.
//
...
{
    //
    // Key and V value are in one array, to allow fast generation of both of
    // them in a single call.
    //
    BYTE keyAndV[32 + 16];
    BYTE previousBlock[16];
    UINT64 requestCounter;  // called reseed_counter in SP 800-90
    BOOLEAN fips140_2Check; // set if the FIPS 140-2 continuous self-test is
                            // required
    SYMCRYPT_MAGIC_FIELD
}
...
typedef const SYMCRYPT_MARVIN32_EXPANDED_SEED*
    PCSYMCRYPT_MARVIN32_EXPANDED_SEED;

typedef SYMCRYPT_MARVIN32_EXPANDED_SEED SYMCRYPT_MARVIN32_CHAINING_STATE,
    *PSYMCRYPT_MARVIN32_CHAINING_STATE;

typedef SYMCRYPT_ALIGN_STRUCT _SYMCRYPT_MARVIN32_STATE
{
    SYMCRYPT_ALIGN BYTE
        buffer[8]; // 4 bytes of data, 4 more bytes for final padding
...
typedef const SYMCRYPT_SP800_108_EXPANDED_KEY*
    PCSYMCRYPT_SP800_108_EXPANDED_KEY;
...
typedef const SYMCRYPT_TLSPRF1_1_EXPANDED_KEY*
    PCSYMCRYPT_TLSPRF1_1_EXPANDED_KEY;
...
#define SYMCRYPT_ASYM_ALIGN_VALUE \
    16 // We have some bugs when ASYM_ALIGN_VALUE > DIGIT_SIZE; need to fix them
       // if we implement AVX2-based x86 code.

#endif

#define SYMCRYPT_ASYM_ALIGN_UP(_p)                         \
    ((PBYTE)(                                              \
        ((UINT_PTR)(_p) + SYMCRYPT_ASYM_ALIGN_VALUE - 1) & \
        ~(SYMCRYPT_ASYM_ALIGN_VALUE - 1)))
...
#define SYMCRYPT_FDEF_DIGITS_FROM_BITS(_bits)       \
    (((_bits) / SYMCRYPT_FDEF_DIGIT_BITS) +         \
     ((((_bits) & (SYMCRYPT_FDEF_DIGIT_BITS - 1)) + \
       (SYMCRYPT_FDEF_DIGIT_BITS - 1)) /            \
...
// that no integer overflow on 32-bit sizes can happen. Note that the computed
// upper bounds are very loose and the actual values are much smaller.
//
#define SYMCRYPT_FDEF_UPB_DIGITS \
    (SYMCRYPT_FDEF_DIGITS_FROM_BITS(SYMCRYPT_INT_MAX_BITS))

//
// All of the following SYMCRYPT_FDEF_SIZEOF_XXX_FROM_YYY computations for the
// four main SymCrypt objects (INT, DIVISOR, MODULUS, MODELEMENT) return a value
// not larger than 2^19 if the inputs _nDigits and _bits are not larger than
// SYMCRYPT_FDEF_UPB_DIGITS and SYMCRYPT_INT_MAX_BITS respectively (For
// MODELEMENT this bound is 2^17). The latter bounds must be enforced on runtime
// for all calculations taking as inputs number of digits or bits.
//
...
// upper 16 bits: bits 16-23: 1-character object type. Bits 24-31: 1 char
// implementation type The upper bits allow objects to be recognized in memory,
...
SYMCRYPT_ASYM_ALIGN_STRUCT _SYMCRYPT_INT
{
    ...
    SYMCRYPT_ASYM_ALIGN union
    {
        struct
        {
            UINT32 uint32[SYMCRYPT_ANYSIZE]; // FDEF: array UINT32[nDigits * #
                                             // uint32 per digit]
        } fdef;
    } ti; // we must have a name here. 'ti' stands for 'Type-Int', it helps
          // catch type errors when type-casting macros are used.
};

#define SYMCRYPT_FDEF_INT_PUINT32(p) (&(p)->ti.fdef.uint32[0])

#define SYMCRYPT_FDEF_SIZEOF_INT_FROM_DIGITS(_nDigits) \
    ((_nDigits)*SYMCRYPT_FDEF_DIGIT_SIZE + sizeof(SYMCRYPT_INT))
#define SYMCRYPT_FDEF_SIZEOF_INT_FROM_BITS(_bits) \
    SYMCRYPT_FDEF_SIZEOF_INT_FROM_DIGITS(SYMCRYPT_FDEF_DIGITS_FROM_BITS(_bits))
...
SYMCRYPT_ASYM_ALIGN_STRUCT _SYMCRYPT_DIVISOR
{
    ...
    union
    {
        struct
        {
            UINT64 W; // approximate inverse of the divisor. Some
                      // implementations will use 64 bits, others 32 bits.
        } fdef;
    } td;
    SYMCRYPT_INT Int; // Having a full Int here uses more space, but allows any
                      // Divisor to still be used as an Int.
    // This structure is directly followed by the Int extension
};

#define SYMCRYPT_FDEF_SIZEOF_DIVISOR_FROM_DIGITS(_nDigits) \
    ((_nDigits)*SYMCRYPT_FDEF_DIGIT_SIZE + sizeof(SYMCRYPT_DIVISOR))
#define SYMCRYPT_FDEF_SIZEOF_DIVISOR_FROM_BITS(_bits) \
    SYMCRYPT_FDEF_SIZEOF_DIVISOR_FROM_DIGITS(         \
        SYMCRYPT_FDEF_DIGITS_FROM_BITS(_bits))
...
SYMCRYPT_ASYM_ALIGN_STRUCT _SYMCRYPT_MODULUS
{
    ...
    union
    {
        struct
        {
            ...
            // UINT32          nUint32Used;    // # 32-bit words used in
            // representing numbers. modulus < 2^{32*nUint32Used}.
            // only values used are nDigits * uint32-per-digit or specific
            // smaller values for optimized implementations
            PCUINT32
                Rsqr; // R^2 mod modulus, in uint32 form, nUint32Used words.
                      // Stored after Divisor. R = 2^{32*nUint32Used}
        } montgomery;
        struct
        {
            UINT32 k; // modulus = 2^<bitsize of modelement> - k
        } pseudoMersenne;
    } tm; // type specific data. Every Modulus can be used as a generic modulus,
          // so no type-specific data for generic.
...
#define SYMCRYPT_FDEF_SIZEOF_MODULUS_FROM_DIGITS(_nDigits) \
    (sizeof(SYMCRYPT_MODULUS) +                            \
     SYMCRYPT_FDEF_SIZEOF_DIVISOR_FROM_DIGITS(_nDigits) +  \
     _nDigits * SYMCRYPT_FDEF_DIGIT_SIZE)
#define SYMCRYPT_FDEF_SIZEOF_MODULUS_FROM_BITS(_bits) \
    SYMCRYPT_FDEF_SIZEOF_MODULUS_FROM_DIGITS(         \
        SYMCRYPT_FDEF_DIGITS_FROM_BITS(_bits))

SYMCRYPT_ASYM_ALIGN_STRUCT _SYMCRYPT_MODELEMENT
{
    // ModElements just store the information without any header. This union
    // makes this well-defined, and allows easy access.
    union
    {
...
#define SYMCRYPT_FDEF_SIZEOF_MODELEMENT_FROM_DIGITS(_nDigits) \
    ((_nDigits)*SYMCRYPT_FDEF_DIGIT_SIZE)
#define SYMCRYPT_FDEF_SIZEOF_MODELEMENT_FROM_BITS(_bits) \
    SYMCRYPT_FDEF_SIZEOF_MODELEMENT_FROM_DIGITS(         \
        SYMCRYPT_FDEF_DIGITS_FROM_BITS(_bits))

    //
    // Upper bound for scratch size computations for FDEF objects depending only
    // on digits
    //
    ...
    //
    // Using the 2^19 (2^17) bound on the sizeof computations the biggest
    // contribution on the above chain is for MODEXP:
    //      ((1 << SYMCRYPT_FDEF_MAX_WINDOW_MODEXP) + 2) *
    //      SYMCRYPT_FDEF_SIZEOF_MODELEMENT_FROM_DIGITS( _nModDigits )
...
#define SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_TO_DIVISOR(_nDigits) \
    (16 * (_nDigits)) // unused currently, but this catches errors

#define SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_MUL(_nDigits) \
    (16 * (_nDigits)) // unused currently, but nonzero size catches errors

#define SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_DIVMOD( \
    _nSrcDigits, _nDivisorDigits)                   \
    ((_nSrcDigits + 1) * SYMCRYPT_FDEF_DIGIT_SIZE)
...
#define SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_EXTENDED_GCD(_nDigits)               \
    (4 * SYMCRYPT_FDEF_SIZEOF_INT_FROM_DIGITS(_nDigits) +                    \
     ...
     SYMCRYPT_MAX(                                                           \
         SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_DIVMOD(2 * _nDigits, _nDigits), \
         SYMCRYPT_MAX(                                                       \
...
#define SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_COMMON_MOD_OPERATIONS(_nModDigits) \
    ((2 * (_nModDigits)*SYMCRYPT_FDEF_DIGIT_SIZE) +                        \
     SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_DIVMOD(                           \
         2 * (_nModDigits),                                                \
         _nModDigits)) // for mult: tmp product + divmod scratch

#define SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_CRT_GENERATION(_nDigits) \
    (2 * SYMCRYPT_FDEF_SIZEOF_INT_FROM_DIGITS(_nDigits) +        \
     SYMCRYPT_MAX(                                               \
         SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_EXTENDED_GCD(_nDigits), \
...
#define SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_TO_MODULUS(_nDigits)  \
    (SYMCRYPT_MAX(                                                \
        SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_TO_DIVISOR(_nDigits), \
        (2 * _nDigits + 1) * SYMCRYPT_FDEF_DIGIT_SIZE +           \
            SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_DIVMOD(           \
                2 * _nDigits + 1, nDigits)))
...
#define SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_MILLER_RABIN(_nDigits)            \
    (SYMCRYPT_FDEF_SIZEOF_MODULUS_FROM_DIGITS(_nDigits) +                     \
     ...
     SYMCRYPT_FDEF_SIZEOF_INT_FROM_DIGITS(_nDigits) +                         \
     SYMCRYPT_MAX(                                                            \
         SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_TO_MODULUS(_nDigits),            \
         SYMCRYPT_MAX(                                                        \
             SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_COMMON_MOD_OPERATIONS(_nDigits), \
...
#define SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_IS_PRIME(_nDigits)            \
    (SYMCRYPT_MAX(                                                        \
        SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_IS_POTENTIAL_PRIME(_nDigits), \
...
#define SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_PRIME_GEN(_nDigits)              \
    (SYMCRYPT_RSAKEY_MAX_NUMOF_PUBEXPS *                                     \
         SYMCRYPT_FDEF_SIZEOF_DIVISOR_FROM_DIGITS(1) +                       \
     SYMCRYPT_FDEF_SIZEOF_INT_FROM_DIGITS(1) +                               \
     SYMCRYPT_MAX(                                                           \
         SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_TO_DIVISOR(1),                  \
         SYMCRYPT_MAX(                                                       \
             SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_DIVMOD(_nDigits, 1),        \
             SYMCRYPT_MAX(                                                   \
                 SYMCRYPT_FDEF_SIZEOF_INT_FROM_DIGITS(_nDigits),             \
                 SYMCRYPT_MAX(                                               \
                     SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_IS_POTENTIAL_PRIME( \
                         _nDigits),                                          \
                     SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_MILLER_RABIN(       \
                         _nDigits))))))
...
// SYMCRYPT_MODMULTIEXP_MAX_NBITSEXP = 2^20. Therefore the upper bound on this
// computation is
//      2^21 + 2^3*(2^6+4)*2^17 + 2^3*2^20*4 < 2^27
//
#define SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_MODMULTIEXP(                          \
    _nModDigits, _nBases, _nBitsExp)                                          \
    (SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_COMMON_MOD_OPERATIONS(_nModDigits) +     \
     ((_nBases) * (1 << SYMCRYPT_FDEF_MAX_WINDOW_MODEXP) + 4) *               \
         SYMCRYPT_FDEF_SIZEOF_MODELEMENT_FROM_DIGITS(_nModDigits) +           \
     (((_nBases) * (_nBitsExp) * sizeof(UINT32) + SYMCRYPT_ASYM_ALIGN_VALUE - \
       1) &                                                                   \
      ~(SYMCRYPT_ASYM_ALIGN_VALUE - 1)))
    // Note: We need +4 mutliplied with
    // SYMCRYPT_FDEF_SIZEOF_MODELEMENT_FROM_DIGITS so that
    // SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_MODMULTIEXP is always at least 2
    // modelements bigger than SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_MODEXP (see
    // modexp.c)
...
#define SYMCRYPT_MASK32_LT(_a, _b) ((UINT32)(((UINT64)(_a) - (_b)) >> 32))
    ...
    // When multiple formats are supported, this is where the information of the
    // multiple formats is combined.
    //
    //  See the comments in SYMCRYPT_FDEF_SCRATCH_XXX regarding 32 bit overflow
    //  protection. All results are bounded above by 2^27.
    //

#define SYMCRYPT_INTERNAL_SIZEOF_INT_FROM_BITS(_bitsize) \
    SYMCRYPT_FDEF_SIZEOF_INT_FROM_BITS(_bitsize)
#define SYMCRYPT_INTERNAL_SIZEOF_DIVISOR_FROM_BITS(_bitsize) \
    SYMCRYPT_FDEF_SIZEOF_DIVISOR_FROM_BITS(_bitsize)
#define SYMCRYPT_INTERNAL_SIZEOF_MODULUS_FROM_BITS(_bitsize) \
    SYMCRYPT_FDEF_SIZEOF_MODULUS_FROM_BITS(_bitsize)
#define SYMCRYPT_INTERNAL_SIZEOF_MODELEMENT_FROM_BITS(_bitsize) \
    SYMCRYPT_FDEF_SIZEOF_MODELEMENT_FROM_BITS(_bitsize)

#define SYMCRYPT_INTERNAL_SIZEOF_RSAKEY_FROM_PARAMS( \
    modBits, nPrimes, nPubExps)                      \
    SYMCRYPT_FDEF_SIZEOF_RSAKEY_FROM_PARAMS(modBits, nPrimes, nPubExps)
    // For now we don't need the pubExpBits so we drop them, but we might use
    // them later.

#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_INT_TO_DIVISOR(_nDigits) \
    SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_TO_DIVISOR(_nDigits)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_INT_MUL(_nDigits) \
    SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_MUL(_nDigits)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_INT_DIVMOD( \
    _nSrcDigits, _nDivisorDigits)                       \
    SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_DIVMOD(_nSrcDigits, _nDivisorDigits)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_EXTENDED_GCD(_nDigits) \
    SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_EXTENDED_GCD(_nDigits)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_COMMON_MOD_OPERATIONS(_nModDigits) \
    SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_COMMON_MOD_OPERATIONS(_nModDigits)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_CRT_GENERATION(_nDigits) \
    SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_CRT_GENERATION(_nDigits)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_CRT_SOLUTION(_nDigits) \
    SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_CRT_SOLUTION(_nDigits)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_INT_TO_MODULUS(_nDigits) \
    SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_TO_MODULUS(_nDigits)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_MODINV(_nModDigits) \
    SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_MODINV(_nModDigits)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_MODEXP(_nModDigits) \
    SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_MODEXP(_nModDigits)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_INT_IS_PRIME(_nDigits) \
    SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_IS_PRIME(_nDigits)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_INT_PRIME_GEN(_nDigits) \
    SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_INT_PRIME_GEN(_nDigits)

#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_MODMULTIEXP( \
    _nModDigits, _nBases, _nBitsExp)                     \
    SYMCRYPT_FDEF_SCRATCH_BYTES_FOR_MODMULTIEXP(_nModDigits, _nBases, _nBitsExp)

//
// RSA padding scratch definitions
//
// The maximum sizes of the state and the result for all hash algorithms are
// sizeof(SYMCRYPT_HASH_STATE) and SYMCRYPT_HASH_MAX_RESULT_SIZE, both not
// bigger 2^20. All the nBytes inputs are bounded by 2^17 (the maximum byte-size
// of the RSA modulus).
//
// Thus a total upper bound on these results is 2^20.
//
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_RSA_OAEP( \
    _hashAlgorithm, _nBytesOAEP)                      \
    (SymCryptHashStateSize(_hashAlgorithm) +          \
     SymCryptHashResultSize(_hashAlgorithm) + 2 * (_nBytesOAEP - 1))

#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_RSA_PKCS1(_nBytesPKCS1) \
    (_nBytesPKCS1)

#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_RSA_PSS(          \
    _hashAlgorithm, _nBytesMessage, _nBytesPSS)               \
    (SymCryptHashStateSize(_hashAlgorithm) + _nBytesMessage + \
     3 * (_nBytesPSS) + 5)
...
#define SYMCRYPT_FDEF_SIZEOF_RSAKEY_FROM_PARAMS(modBits, nPrimes, nPubExps) \
    ...
    // 1 int per pubexp for each privexp +  1 int per prime*pubexp for each
    // crtprivexp
...
#define SYMCRYPT_RSAKEY_MIN_BITSIZE_MODULUS \
    (256) // Some of our SCS code requires at least 32 bytes...
#define SYMCRYPT_RSAKEY_MAX_BITSIZE_MODULUS \
    (1 << 16) // Avoid any integer overflows in size calculations
#define SYMCRYPT_RSAKEY_MIN_BITSIZE_PRIME (128)
#define SYMCRYPT_RSAKEY_MAX_BITSIZE_PRIME \
    (SYMCRYPT_RSAKEY_MAX_BITSIZE_MODULUS / 2)
...
typedef SYMCRYPT_ASYM_ALIGN_STRUCT _SYMCRYPT_RSAKEY
{
    ...
    BOOLEAN
        hasPrivateKey; // Set to true if there is private key information set

    UINT32 nSetBitsOfModulus; // Bits of modulus specified during creation

    UINT32 nBitsOfModulus; // Number of bits of the value of the modulus (not
                           // the object's size)
    UINT32
        nDigitsOfModulus; // Number of digits of the modulus object (alwaays
                          // equal to SymCryptDigitsFromBits(nSetBitsOfModulus))

    UINT32 nPubExp; // Number of public exponents

    UINT32 nPrimes; // Number of primes, can be 0 if the object only supports
                    // public keys
    ...
    // SYMCRYPT_ASYM_ALIGN'ed buffers that point to memory allocated for each
    // object
    ...
    PBYTE pbCrtPrivExps
        [SYMCRYPT_RSAKEY_MAX_NUMOF_PUBEXPS * SYMCRYPT_RSAKEY_MAX_NUMOF_PRIMES];
...
//
// The following definitions relating to trial divisoin are not needed by normal
// callers but are used by the test program to measure performance of
// components.
//
...
//
// This structure is used to test whether a UINT64 is a multiple of a (small)
// prime. Let V be the input value, P the small prime, and W the inverse of P
// modulo 2^64. If V = k*P then V * M mod 2^64 = V/P mod 2^64 = k. This holds
// for k = 0, 1, ..., floor( (2^{64}-1)/p ). If V is not a multiple of P then
// the result of the multiplication must be larger than that.
//
...
typedef struct _SYMCRYPT_TRIALDIVISION_GROUP
{
    ...
    UINT32
        factor[9]; // factors[i] = 2^{32*(i+1)} mod Prod where Prod = product of
                   // the primes It is guaranteed that Prod <= (2^{32}-1)/9
...
typedef struct _SYMCRYPT_TRIALDIVISION_CONTEXT
{
    ...
    SYMCRYPT_TRIALDIVISION_PRIME
        Primes3_5_17[3]; // Structures for 3, 5 and 17 in that order
...
SymCryptTestTrialdivisionMaxSmallPrime(
    PCSYMCRYPT_TRIALDIVISION_CONTEXT
        pContext); // Expose small prime limit to help test code
...
// Minimum allowable bit sizes for generated and imported parameters for both P
// and Q primes.
...
typedef SYMCRYPT_ASYM_ALIGN_STRUCT _SYMCRYPT_DLGROUP
{
    ...
    BOOLEAN
        fHasPrimeQ; // Flag that specifies whether the object has a Q parameter

    UINT32 nBitsOfP; // Number of bits of the value of P (not the object's size)
    UINT32 cbPrimeP; // Number of bytes of the value of P (not the object's
                     // size), equal to ceil(nBitsOfP/8)
    ...
    UINT32 cbPrimeQ; // Number of bytes of the value of Q (not the object's
                     // size), equal to ceil(nBitsOfQ/8)
    UINT32 nDigitsOfQ;  // Number of digits of the object of prime Q
    UINT32 nMaxBitsOfQ; // Maximum number of bits of the value of Q

    BOOLEAN
        isSafePrimeGroup; // Boolean indicating if this is a Safe Prime group
    UINT32 nBitsPriv;     // Number of bits in private keys using this group
                      // Normally equals nBitsOfQ, but may be further restricted
                      // (i.e. for named Safe Prime groups)

    UINT32 nBitsOfSeed; // Number of bits of the seed used for generation
                        // (seedlen in FIPS 186-3)
    UINT32 cbSeed; // Number of bytes of the seed, equal to ceil(nBitsOfSeed/8)

    SYMCRYPT_DLGROUP_FIPS
        eFipsStandard; // Code specifying the FIPS standard used to create the
                       // keys. If 0 the group is unverified.

    PCSYMCRYPT_HASH
        pHashAlgorithm;  // Hash algorithm used for the generation of parameters
    UINT32 dwGenCounter; // Number of iterations used for the generation of
                         // parameters
    BYTE bIndexGenG;     // Index for the generation of generator G (FIPS 186-3)
                         // (Always 1 for now)

    PBYTE pbQ; // SYMCRYPT_ASYM_ALIGN'ed buffer that points to the memory
               // allocated for modulus Q
    ...
    PBYTE pbSeed; // Buffer that will hold the seed (this is padded at the end
                  // so that the entire structure has size a multiple of
...
typedef SYMCRYPT_ASYM_ALIGN_STRUCT _SYMCRYPT_DLKEY
{
    ...
    BOOLEAN fPrivateModQ;   // Set to true if the private key is at most Q-1,
                            // otherwise it is at most P-2

    PBYTE pbPrivate; // SYMCRYPT_ASYM_ALIGN'ed buffer that points to the memory
                     // allocated for the private key
    ...
    // PublicKey
    // PrivateKey                           // The size of this must always be
    // the same as the size of P
}
SYMCRYPT_DLKEY;
...
#define SYMCRYPT_ECPOINT_FORMAT_MAX_LENGTH \
    4 // Number of MODELEMENTs for the largest ECPOINT format
...
typedef enum _SYMCRYPT_ECPOINT_COORDINATES
{
    ...
    SYMCRYPT_ECPOINT_COORDINATES_PROJECTIVE =
        0x33, // Three equally-sized values where the triple (X,Y,Z) represents
              // the affine point (X/Z, Y/Z)
    SYMCRYPT_ECPOINT_COORDINATES_JACOBIAN =
        0x43, // Three equally-sized values where the triple (X,Y,Z) represents
              // the affine point (X/Z^2, Y/Z^3)
    SYMCRYPT_ECPOINT_COORDINATES_EXTENDED_PROJECTIVE =
        0x54, // Four equally-sized values where (X,Y,Z,T) represents the affine
              // point (X/Z, Y/Z) with T=X*Y*Z
    SYMCRYPT_ECPOINT_COORDINATES_SINGLE_PROJECTIVE =
        0x62, // Two equally-sized values where (X,Z) represents the point (X/Z)
...
#define SYMCRYPT_ECURVE_SW_DEF_WINDOW \
    (6) // Default window size for the windowed methods

#define SYMCRYPT_ECURVE_SW_MAX_NPRECOMP_POINTS \
    (64) // Maximum number of precomputed points
...
typedef SYMCRYPT_ASYM_ALIGN_STRUCT _SYMCRYPT_ECURVE
{
    ...
    UINT32 FModBytesize; // Bytesize of the field modulus (specified in the
                         // curve parameters as cbFieldLength)

    UINT32 GOrdBitsize;  // Bitsize of the (sub)group order
    UINT32 GOrdDigits;   // Number of digits of the (sub)group order
    UINT32 GOrdBytesize; // Bytesize of the (sub)group order (specified in the
                         // curve parameters as cbSubgroupOrder)
    ...
    UINT32
        cbScratchCommon; // Size of scratch space for common ecurve operations
    UINT32 cbScratchScalar; // Size of constant scratch space for scalar ecurve
                            // operations (without the nPoints dependence)
    UINT32 cbScratchScalarMulti; // Dependence of scratch space for scalar
                                 // ecurve operations from nPoints
    UINT32 cbScratchGetSetValue; // Size of scratch space for get set value
                                 // ecpoint operations
    UINT32 cbScratchEckey;       // Size of scratch space for eckey operations

    UINT32 coFactorPower; // The cofactor of the curve will be equal to
                          // 2^coFactorPower
    ...
    {
        SYMCRYPT_ECURVE_INFO_PRECOMP
            sw; // Info for short Weierstrass curves (only the precomputation
                // parameters are needed now)
...
#define SYMCRYPT_INTERNAL_ECPOINT_COORDINATE_OFFSET(_pCurve, _ord) \
    (sizeof(SYMCRYPT_ECPOINT) + (_ord) * (_pCurve)->cbModElement)
#define SYMCRYPT_INTERNAL_ECPOINT_COORDINATE(_ord, _pCurve, _pEcpoint) \
    (PSYMCRYPT_MODELEMENT)(                                            \
        (PBYTE)(_pEcpoint) +                                           \
        SYMCRYPT_INTERNAL_ECPOINT_COORDINATE_OFFSET((_pCurve), _ord))

//
// Scratch space sizes for ECURVE operations
//
//  Overflow protection is enforced when creating the ECURVE objects on
//  the cbScratchCommon, cbScratchScalar, cbScratchScalarMulti, and
//  cbScratchEckey fields.
//
//  All of them are upper bounded by 2^26 (see
//  SymCrypt<CurveType>FillScratchSpaces functions) and since _nPoints is
//  bounded by SYMCRYPT_ECURVE_MULTI_SCALAR_MUL_MAX_NPOINTS = 2, all the macros
//  are bounded by 2^27.
//

#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_COMMON_ECURVE_OPERATIONS(_pCurve) \
    ((_pCurve)->cbScratchCommon)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_SCALAR_ECURVE_OPERATIONS( \
    _pCurve, _nPoints)                                                \
    ((_pCurve)->cbScratchScalar + (_nPoints) * (_pCurve)->cbScratchScalarMulti)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_GETSET_VALUE_ECURVE_OPERATIONS( \
    _pCurve)                                                                \
    ((_pCurve)->cbScratchGetSetValue)
#define SYMCRYPT_INTERNAL_SCRATCH_BYTES_FOR_ECKEY_ECURVE_OPERATIONS(_pCurve) \
    ((_pCurve)->cbScratchEckey)

typedef SYMCRYPT_ASYM_ALIGN_STRUCT _SYMCRYPT_ECPOINT
{
    BOOLEAN normalized; // A flag specifying whether the point is normalized or
                        // not. This flag makes sense only for PROJECTIVE,
                        // JACOBIAN, EXTENDED_PROJECTIVE, and SINGLE_PROJECTIVE
                        // coordinates. If set to TRUE (non-zero), it means that
                        // the Z coordinate of the point is equal to 1.
    PCSYMCRYPT_ECURVE pCurve; // Handle to the curve which the point is on. Only
                              // used in CHKed builds for ASSERTs
    SYMCRYPT_MAGIC_FIELD
    // An array of MODELEMENTs. The total size will depend on the MODELEMENT
    // size and the number of MODELEMENTs.
}
...
#define SYMCRYPT_EXTERN_C_END
    ...
    //
    // Callers of SymCrypt should NOT depend on the function names in these
    // macros. The definition of these macros can change in future releases of
    // the library.
    //

#if SYMCRYPT_CPU_X86 | SYMCRYPT_CPU_AMD64
typedef struct _SYMCRYPT_EXTENDED_SAVE_DATA SYMCRYPT_EXTENDED_SAVE_DATA,
    *PSYMCRYPT_EXTENDED_SAVE_DATA;

#define SYMCRYPT_ENVIRONMENT_DEFS_SAVEYMM(envName)            \
    SYMCRYPT_ERROR SYMCRYPT_CALL SymCryptSaveYmmEnv##envName( \
        _Out_ PSYMCRYPT_EXTENDED_SAVE_DATA pSaveArea);        \
    SYMCRYPT_ERROR SYMCRYPT_CALL SymCryptSaveYmm(             \
        _Out_ PSYMCRYPT_EXTENDED_SAVE_DATA pSaveArea)         \
    {                                                         \
        return SymCryptSaveYmmEnv##envName(pSaveArea);        \
    }                                                         \
                                                              \
    VOID SYMCRYPT_CALL SymCryptRestoreYmmEnv##envName(        \
        _Inout_ PSYMCRYPT_EXTENDED_SAVE_DATA pSaveArea);      \
    VOID SYMCRYPT_CALL SymCryptRestoreYmm(                    \
        _Inout_ PSYMCRYPT_EXTENDED_SAVE_DATA pSaveArea)       \
    {                                                         \
...
#define SYMCRYPT_ENVIRONMENT_DEFS_SAVEXMM(envName)            \
    SYMCRYPT_ERROR SYMCRYPT_CALL SymCryptSaveXmmEnv##envName( \
        _Out_ PSYMCRYPT_EXTENDED_SAVE_DATA pSaveArea);        \
    SYMCRYPT_ERROR SYMCRYPT_CALL SymCryptSaveXmm(             \
        _Out_ PSYMCRYPT_EXTENDED_SAVE_DATA pSaveArea)         \
    {                                                         \
        return SymCryptSaveXmmEnv##envName(pSaveArea);        \
    }                                                         \
                                                              \
    VOID SYMCRYPT_CALL SymCryptRestoreXmmEnv##envName(        \
        _Inout_ PSYMCRYPT_EXTENDED_SAVE_DATA pSaveArea);      \
    VOID SYMCRYPT_CALL SymCryptRestoreXmm(                    \
        _Inout_ PSYMCRYPT_EXTENDED_SAVE_DATA pSaveArea)       \
    {                                                         \
...
#define SYMCRYPT_ENVIRONMENT_FORWARD_CPUIDEX(envName)                          \
    VOID SYMCRYPT_CALL SymCryptCpuidExFuncEnv##envName(                        \
        int cpuInfo[4], int function_id, int subfunction_id);                  \
    VOID SYMCRYPT_CALL SymCryptCpuidExFunc(                                    \
        int cpuInfo[4], int function_id, int subfunction_id)                   \
    {                                                                          \
...
#define SYMCRYPT_ENVIRONMENT_DEFS(envName)                                 \
    ...
    {                                                                      \
        SymCryptInitEnv##envName(SYMCRYPT_API_VERSION);                    \
                                                                           ...
                                                                           \
    _Analysis_noreturn_ VOID SYMCRYPT_CALL SymCryptFatalEnv##envName(      \
        UINT32 fatalCode);                                                 \
    ...
    SYMCRYPT_CPU_FEATURES SYMCRYPT_CALL                                    \
        SymCryptCpuFeaturesNeverPresentEnv##envName();                     \
    ...
    SYMCRYPT_ENVIRONMENT_DEFS_SAVEYMM(envName)                             \
                                                                           \
    VOID SYMCRYPT_CALL SymCryptTestInjectErrorEnv##envName(                \
        PBYTE pbBuf, SIZE_T cbBuf);                                        \
...
//
// To avoid hard-do-diagnose mistakes, we skip defining environment macros in
// those cases where we know they cannot or should not be used.
//

#define SYMCRYPT_ENVIRONMENT_GENERIC SYMCRYPT_ENVIRONMENT_DEFS(Generic)

#if defined(EFI) | defined(PCAT) | defined(DIRECT)
#define SYMCRYPT_ENVIRONMENT_WINDOWS_BOOTLIBRARY \
    SYMCRYPT_ENVIRONMENT_DEFS(WindowsBootlibrary)
#endif

//
// There are no defined symbols that we can use to detect that we are in
// debugger code But this is unlikely to be misued.
//
#define SYMCRYPT_ENVIRONMENT_WINDOWS_KERNELDEBUGGER \
    SYMCRYPT_ENVIRONMENT_DEFS(WindowsKernelDebugger)

#define SYMCRYPT_ENVIRONMENT_WINDOWS_KERNELMODE_LEGACY \
    SYMCRYPT_ENVIRONMENT_GENERIC

#if (NTDDI_VERSION >= NTDDI_WIN7)
#define SYMCRYPT_ENVIRONMENT_WINDOWS_KERNELMODE_WIN7_N_LATER \
    SYMCRYPT_ENVIRONMENT_DEFS(WindowsKernelmodeWin7nLater)
#endif

#if (NTDDI_VERSION >= NTDDI_WINBLUE)
#define SYMCRYPT_ENVIRONMENT_WINDOWS_KERNELMODE_WIN8_1_N_LATER \
    SYMCRYPT_ENVIRONMENT_DEFS(WindowsKernelmodeWin8_1nLater)
#endif

#define SYMCRYPT_ENVIRONMENT_WINDOWS_KERNELMODE_LATEST \
...
#if (NTDDI_VERSION >= NTDDI_WIN7)
#define SYMCRYPT_ENVIRONMENT_WINDOWS_USERMODE_WIN7_N_LATER \
    SYMCRYPT_ENVIRONMENT_DEFS(WindowsUsermodeWin7nLater)
#endif

#if (NTDDI_VERSION >= NTDDI_WINBLUE)
#define SYMCRYPT_ENVIRONMENT_WINDOWS_USERMODE_WIN8_1_N_LATER \
    SYMCRYPT_ENVIRONMENT_DEFS(WindowsUsermodeWin8_1nLater)
#endif

#if (NTDDI_VERSION >= NTDDI_WIN10)
#define SYMCRYPT_ENVIRONMENT_WINDOWS_USERMODE_WIN10_SGX \
    SYMCRYPT_ENVIRONMENT_DEFS(Win10Sgx)
#endif

#define SYMCRYPT_ENVIRONMENT_WINDOWS_USERMODE_LATEST \
    SYMCRYPT_ENVIRONMENT_WINDOWS_USERMODE_WIN8_1_N_LATER

#define SYMCRYPT_ENVIRONMENT_LINUX_USERMODE \
    SYMCRYPT_ENVIRONMENT_DEFS(LinuxUsermode)
...
// This is a tradeoff between speed and code size and there are diminishing
// returns to supporting increasingly large sizes. We currently put the limit at
// ~8 native writes, which varies by platform.
//
#if SYMCRYPT_CPU_X86 | SYMCRYPT_CPU_ARM
#define SYMCRYPT_WIPE_FUNCTION_LIMIT \
    (32) // If this is increased beyond 127 the code below must be updated.
#elif SYMCRYPT_CPU_AMD64 | SYMCRYPT_CPU_ARM64
#define SYMCRYPT_WIPE_FUNCTION_LIMIT \
    (64) // If this is increased beyond 127 the code below must be updated.
#else
#error ??
#endif
...
#pragma prefast(     \
    suppress : 6101, \
    "Logic why this properly initializes the pbData buffer is too complicated for prefast")
SymCryptWipeKnownSize(_Out_writes_bytes_(cbData) PVOID pbData, SIZE_T cbData)
{
    ...
    {
        //
        // We assume that pb is aligned, so we wipe from the end to the front to
        // keep alignment.
        //
        if (cbData & 1)
        {
        ...
        if (cbData & 16)
        {
            cbData -= 16;
            SYMCRYPT_INTERNAL_FORCE_WRITE64((volatile UINT64*)&pb[cbData], 0);
            SYMCRYPT_INTERNAL_FORCE_WRITE64(
                (volatile UINT64*)&pb[cbData + 8], 0);
        }
        if (cbData & 32)
        {
            cbData -= 32;
            ...
            SYMCRYPT_INTERNAL_FORCE_WRITE64(
                (volatile UINT64*)&pb[cbData + 8], 0);
            SYMCRYPT_INTERNAL_FORCE_WRITE64(
                (volatile UINT64*)&pb[cbData + 16], 0);
            SYMCRYPT_INTERNAL_FORCE_WRITE64(
                (volatile UINT64*)&pb[cbData + 24], 0);
        }
        ...
        if (cbData & 64)
        {
            ...
            SYMCRYPT_INTERNAL_FORCE_WRITE64(
                (volatile UINT64*)&pb[cbData + 24], 0);
            SYMCRYPT_INTERNAL_FORCE_WRITE64(
                (volatile UINT64*)&pb[cbData + 32], 0);
            SYMCRYPT_INTERNAL_FORCE_WRITE64(
                (volatile UINT64*)&pb[cbData + 40], 0);
            SYMCRYPT_INTERNAL_FORCE_WRITE64(
                (volatile UINT64*)&pb[cbData + 48], 0);
            SYMCRYPT_INTERNAL_FORCE_WRITE64(
                (volatile UINT64*)&pb[cbData + 56], 0);
        }
#endif
    }
}
